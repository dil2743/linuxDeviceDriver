...
int pcd_platform_device_probe(struct platform_device *pdev) {
    ...
    struct pcdev_private_data *dev_data = kzalloc(sizeof(struct pcdev_private_data), GFP_KERNEL);
    if (!dev_data) {
    ...
    dev_data->pdata = kzalloc(sizeof(struct pcdev_platform_data), GFP_KERNEL);
    if (!dev_data->pdata) {
    ...
    pdev->dev.platform_data = dev_data->pdata;
